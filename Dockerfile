...
RUN --mount=type=cache,id=s/9a8c909a-711c-41ea-bd56-acb9405c7ee0-/root/.local/share/pnpm/store,target=/root/.local/share/pnpm/store,sharing=locked \
...
RUN --mount=type=cache,id=s/9a8c909a-711c-41ea-bd56-acb9405c7ee0-/var/cache/apt,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=s/9a8c909a-711c-41ea-bd56-acb9405c7ee0-/var/lib/apt,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
EXPOSE 18789
CMD ["sh", "-c", "node openclaw.mjs gateway --allow-unconfigured --port ${PORT:-18789} --bind 0.0.0.0"]
